...
## ステップ6: コードをリファクタリング（REFACTOR）

テストをグリーンに保ちながらコードを改善します。この例では実装がシンプルなためリファクタリング不要です。
...
- ❌ 失敗するテストを無視
- ❌ 実装の詳細をテスト（動作をテスト）
- ❌ 外部依存もすべてモック化する（外部HTTP/DBは統合テストで実際に検証すること）
...
**必須**: テストは実装前に書かなければなりません（上記「TDDサイクル」参照）。
